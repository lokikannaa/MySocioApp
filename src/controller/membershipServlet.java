...
//            User theUser = UserDB.select(email,password); 
            ...
            System.out.println("the user last login time " +theUser.getLastLoginTime()); 
...
//            session.setAttribute("viewtweets", viewtweets);
            ...
            url = "/login.jsp";//"/home.jsp";    the "home" page
            }
            request.setAttribute("message", message);
             }catch (NoSuchAlgorithmException ex) {
             System.out.println(ex);
      ...
      // update the password in the db
           ...
           // send email to user
            ...
            String from="mysocioApp";
            String subject = "Forgot password e-mail";
...
//        }
         ...
         // Logic for inserting the followers in Follow table 
          ...
          {
              String insertFollowTime = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss").format(Calendar.getInstance().getTime());
         ...
         //     Follow nonFollowUser = new Follow(currUser.getUserId(),nfUser.getUserId(),insertFollowTime); 
